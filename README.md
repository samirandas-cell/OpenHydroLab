...
| `animations/Lec11_unit_hydrograph.html` | Unit Hydrograph Workbench — three modes on one gamma-IUH foundation: derive UH from an observed storm (interactive baseflow separation + rain-gauge reconciliation check), apply UH by superposition (stacked component bands), change duration via S-curve lag-and-subtract (continuous D′ slider, volume conserved at 1 cm) | 2D, fully offline |
