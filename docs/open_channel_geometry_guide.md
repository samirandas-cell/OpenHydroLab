...
Animation: `animations/Lec1_open_channel_geometry.html` (3D, Three.js via CDN — needs
internet on first load; everything else is self-contained in the single file).

Supports CVE2142 Lecture 1 (*Introduction to Open Channel Hydraulics*), Sections 3–7.
...
3. **What does the Froude number physically mean?** Not "V over root-g-D", but: *can a
   ...
   it is why subcritical flow is controlled from downstream (backwater, Lecture 5) and
...
Derived: hydraulic radius R = A/P (friction length scale, → Manning, Lecture 2) and
...
### Verified numbers (against the Lecture 1 worked examples)
...
- Uniform-flow scene: the water surface is drawn parallel to the bed (normal-depth
  condition); no GVF surface curvature (that is Lecture 5's animation).
...
- Das, S. (2025). CVE2142 Hydraulics and Hydrology, Lecture 1, University of Glasgow
