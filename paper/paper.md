...
<https://github.com/samirandas-cell/OpenHydroLab>, and version 1.0.3 — the version
described and verified in this paper — is archived at
[10.5281/zenodo.21904490](https://doi.org/10.5281/zenodo.21904490). The concept DOI
...
validation dataset (`validation/results/`), so every number reported in this paper can be
regenerated by running `npm install && npm run install-browsers && npm test` against the
archived code, followed by `node tools/manuscript-tables.mjs`. One qualification: the
dataset reported here is the run recorded at commit `36a9733`, which adds three swept
invariant cases to the archived v1.0.3 snapshot, so running the suite against the archive
reproduces every table except the three rows those cases contribute (Table 6, the
hydrostatic laboratories) and the totals that include them. The rainfall record embedded
in the frequency laboratory is Met Office MIDAS-Open hourly data for gauge Drumalbin 00987,
used under the UK Open Government Licence.
...
(Version 1.0.3) [Computer software]. <https://doi.org/10.5281/zenodo.21904490>
...
<https://doi.org/10.5281/zenodo.21904490> and is regenerated by the test suite. No other
data were used.
