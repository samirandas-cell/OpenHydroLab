---
title: "OpenChannelLab: interactive, physics-accurate animations for teaching open-channel hydraulics"
tags:
...
OpenChannelLab is a library of interactive, single-file HTML animations for teaching
undergraduate open-channel hydraulics. Each animation computes every displayed quantity
live from the governing equations — continuity, specific energy, momentum, the Manning
equation, and the gradually-varied-flow (GVF) equation — so students manipulate real
physics rather than pre-rendered illustrations. The current release covers channel
geometry and the Froude number, Manning uniform flow with normal/critical depth solvers,
the specific-energy diagram with sluice-gate choking, hydraulic jumps (all five types,
momentum-based sequent depths, energy loss against theory), and numerical GVF integration
with automatic classification of all twelve profile types. Animations run in any modern
browser with no installation; all but one are fully offline, making them robust for
classroom projection, virtual learning environments, and independent student study.
...
their governing equations. OpenChannelLab fills this gap with instruments that are
