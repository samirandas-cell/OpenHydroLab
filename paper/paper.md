...
described and verified here — is archived at
...
release. The archived snapshot contains the verification protocol
(`validation/validation-protocol.md`) and the generated validation dataset
(`validation/results/`), so every number reported in this paper can be regenerated by
running `npm test` against the archived code.
